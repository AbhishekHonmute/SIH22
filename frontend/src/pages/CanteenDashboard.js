...
import { Checkbox, colors, Menu, Table, TableBody, TableCell, TableContainer, TableHead, TableRow } from "@mui/material";
...
import veg_non_veg from '../images/veg_non_veg.jpeg'
...
class CanteenDashboard extends Component {
  ...
  componentDidMount(){
    ...
    axios.get(`${backendURL}/get_canteen/${canteen_id}`)
      .then(resp => {
      ...
      axios.get(`${backendVenueURL}/get_venues`)
      .then(resp => {
  ...
  render() {
    ...
    >
      <Grid container spacing={8}>
        <Grid item>
          <ButtonBase sx={{ width: 200, height: 200 }}>
            <Img alt="complex" src="https://img.freepik.com/premium-vector/school-canteen-interior-flat-illustration_318844-69.jpg" />
          </ButtonBase>
        </Grid>
        <Grid item xs={12} sm container>
          <Grid item xs container direction="column" spacing={2}>
            <Grid item xs>
              <Typography gutterBottom variant="body1" component="div"  suppressContentEditableWarning={this.state.is_profile_editable} contentEditable={this.state.is_profile_editable} onChange={this.handleChange}>
                
                {this.state.is_profile_editable?
                <TextField id="standard-basic" label="name" name="name" variant="standard" value={this.state.name} onChange={this.handleChange}/>
                :
                <>{this.state.name}</>
                }
              ...
              <Typography gutterBottom variant="subtitle2" component="div"  suppressContentEditableWarning={this.state.is_profile_editable} contentEditable={this.state.is_profile_editable} onChange={this.handleChange}>
                {this.state.is_profile_editable?
                <TextField id="standard-basic" label="phone_no" name="phone_no" variant="standard" value={this.state.phone_no} onChange={this.handleChange}/>
                :
                <>{this.state.phone_no}</>
                }
              </Typography>
              ...
              <Typography gutterBottom variant="subtitle1" component="div"  suppressContentEditableWarning={this.state.is_profile_editable} contentEditable={this.state.is_profile_editable} onChange={this.handleChange}>
                {this.state.is_profile_editable?
                <TextField id="standard-basic" label="email" name="email" variant="standard" value={this.state.email} onChange={this.handleChange}/>
                :
                <>{this.state.email}</>
                }
              </Typography>
            ...
            <Grid>
              {this.state.is_profile_editable?
              <Box>
                {this.state.allvenue.map((venue) => <Grid  key={venue[0]} item >
            ...
            <Grid item>
              ...
              {this.state.is_profile_editable?
              <Button variant="outlined" color="success" onClick={this.handleSaveProfile} >
              Save
              </Button> 
              :
      ...
      {this.state.showmenu?
        <Grid>
        <Button variant="outlined" color="success" onClick={this.handleShowMenucard} >
          Hide MenuCard
        </Button>
        
        <Button onClick={this.handleAddfooditem}><PostAddIcon></PostAddIcon> Add Menu</Button>
        </Grid>
      :
        <Grid>
          <Button variant="outlined" color="success" onClick={this.handleShowMenucard} >
        ...
        >
        <Typography variant="h2" align="center">{this.state.name}</Typography>
        <Grid>
        {this.state.menu.map((fooditem) => (<Menufooditem key={fooditem[0]} item={fooditem} canteen_id={this.state.canteen_id} ></Menufooditem>))}
        </Grid>
        </Box>
        :
        <></>
